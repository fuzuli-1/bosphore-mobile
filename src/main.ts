...
bootstrapApplication(AppComponent, {
  ...
  providers: [
   ...
   // 1. ÖNCE prefix'i set et
    console.log('Bootstrap başlıyor...'); // Debug için
    const configService = app.injector.get(ApplicationConfigService);
    configService.setEndpointPrefix(GeneralSettings.url);

    // 2. SONRA çeviriyi yükle
    const ts = app.injector.get(TranslationService);
    const storageService = app.injector.get(StateStorageService);
    storageService.storeLocale('fr');
    return ts.use('fr');
